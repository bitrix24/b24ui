 ...
 */
export default function TemplatePlugin(options: Bitrix24UIOptions, componentDir?: string) {
  ...
  const templates = getTemplates(options, undefined, undefined, vue)
    ...
    vite: {
      async config(config) {
        ...
        if (options.experimental?.componentDetection && componentDir) {
          // `scanPackages` packages resolve b24ui components from `node_modules`
          ...
          if (vue.detectedComponents?.size) {
            consola.success(`Bitrix24 UI detected ${vue.detectedComponents.size} components in use (including dependencies)`)
          } else {
            consola.info('Bitrix24 UI detected no components in use, including all components')
          }
        }
