...
export function devtoolsMetaPlugin({ resolve, options, templates }: { resolve: Resolver['resolve'], options: ModuleOptions, templates: Record<string, any> }) {
  return {
    ...
    configureServer(server: ViteDevServer) {
      server.middlewares.use('/__bitrix24_ui__/devtools/api/component-meta', async (_req, res) => {
        res.setHeader('Content-Type', 'application/json')
        try {
          // @memo shevchik fix
          const componentMeta = await import('./.component-meta/.nuxt/component-meta')
      ...
      server.middlewares.use('/__bitrix24_ui__/devtools/api/component-example', async (req, res) => {
