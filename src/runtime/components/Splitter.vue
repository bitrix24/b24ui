...
export interface SplitterItem extends Omit<SplitterPanelProps, 'as' | 'asChild'> {
  ...
  slot?: string
  class?: any
  b24ui?: Partial<Pick<Splitter['slots'], 'panel'>>
  [key: string]: any
}
...
export type SplitterSlots<T extends SplitterItem = SplitterItem> = {
  'resize-handle'?: (props: { index: number, b24ui: Splitter['b24ui'] }) => VNode[]
...
const b24ui = computed(() => tv({ extend: theme, ...(appConfig.b24ui?.splitter || {}) })({
...
<template>
  <SplitterGroup v-bind="rootProps" :direction="props.orientation!" data-slot="root" :class="b24ui.root({ class: [props.b24ui?.root, props.class] })" @layout="emits('layout', $event)">
    <template v-for="(item, index) in props.items" :key="item.id ?? index">
      <SplitterPanel
        ...
        :order="item.order"
        data-slot="panel"
        :class="b24ui.panel({ class: [props.b24ui?.panel, item.b24ui?.panel, item.class] })"
      ...
      >
        <slot name="resize-handle" :index="index" :b24ui="b24ui" />
      </SplitterResizeHandle>
    </template>
  </SplitterGroup>
</template>
