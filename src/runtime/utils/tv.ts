 ...
 * (`:b24ui` / `class`, which includes what `<B24Theme>` injects) drops the slot's
...
/**
 * Resolve construction-time replacers authored in `app.config.b24ui.<component>`
 ...
 * has no way to remove them. Call-time replacers (`:b24ui` / `class`) run after
