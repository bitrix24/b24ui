<script setup lang="ts">
import theme from '#build/b24ui/avatar'

const sizes = Object.keys(theme.variants.size) as Array<keyof typeof theme.variants.size>
</script>

<template>
  <div class="flex flex-col gap-2 items-center">
    <div class="flex items-center gap-1.5">
      <B24Avatar v-for="size in sizes" :key="size" src="https://github.com/benjamincanac.png" alt="Benjamin Canac" :size="size" />
    </div>
    <div class="flex items-center gap-1.5">
      <B24Avatar v-for="size in sizes" :key="size" icon="i-lucide-image" :size="size" />
    </div>
    <div class="flex items-center gap-1.5">
      <B24Avatar v-for="size in sizes" :key="size" alt="Benjamin Canac" :size="size" />
    </div>
    <div class="flex items-center gap-1.5">
      <B24Avatar v-for="size in sizes" :key="size" :text="size" :size="size" />
    </div>

    <div class="flex items-center gap-1.5">
      <B24AvatarGroup v-for="size in sizes" :key="size" :size="size" :max="2">
        <B24Chip inset text="1">
          <B24Avatar src="https://github.com/benjamincanac.png" alt="Benjamin Canac" />
        </B24Chip>
        <B24Avatar src="https://github.com/romhml.png" alt="Romain Hamel" />
        <B24Avatar src="https://github.com/noook.png" alt="Neil Richter" />
      </B24AvatarGroup>
    </div>
    <div class="flex items-center gap-1.5">
      <B24AvatarGroup v-for="size in sizes" :key="size" :size="size" :max="4">
        <B24Avatar src="https://github.com/benjamincanac.png" alt="Benjamin Canac" />
        <B24Avatar src="https://github.com/romhml.png" alt="Romain Hamel" />
        <B24Avatar src="https://github.com/noook.png" alt="Neil Richter" />
      </B24AvatarGroup>
    </div>
  </div>
</template>
