 ...
 * `<B24Theme :props>` and `app.config` defaults.
 *
...
const noBarePropRefs = {
  meta: {
    ...
    schema: [],
    messages: {
      bareRef: 'Bare prop reference `{{ name }}` bypasses the `useComponentProps` proxy. Use `{{ propsVar }}.{{ name }}` so `<B24Theme :props>` defaults flow through.'
    }
  },
  create(context) {
 ...
 * wrapping `<B24Form>` / `<B24FormField>` / `<B24FieldGroup>` supplied, so a
 * bare `size.value` silently drops `<B24Theme :props>` and `app.config`
 * defaults. The fix is always the same shape, either inline or hoisted into a
 * computed:
 *
 * ```ts
...
const noUnresolvedFormFieldRefs = {
  meta: {
    ...
    schema: [],
    messages: {
      unresolved: 'Reading `{{ local }}` without a `?? {{ propsVar }}.{{ key }}` fallback drops `<B24Theme :props>` and `app.config` defaults. Chain it, or read a computed that already does.',
      inTemplate: 'Binding the raw `{{ local }}` in the template drops `<B24Theme :props>` and `app.config` defaults, and refs auto-unwrap here so it is indistinguishable from a resolved one. Resolve it in setup with `computed(() => {{ local }}.value ?? {{ propsVar }}.{{ key }})` and bind that.'
    }
  },
  create(context) {
    const parserServices = context.sourceCode?.parserServices ?? context.parserServices
