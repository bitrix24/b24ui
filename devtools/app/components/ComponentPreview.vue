...
const { data: componentExample } = useAsyncData('__b24ui_devtools_component-source', async () => {
...
const { data: formattedCode } = useAsyncData('__b24ui-devtools-component-formatted-code', async () => {
...
const { data: highlightedCode } = useAsyncData('__b24ui-devtools-component-highlighted-code', async () => {
...
function onRendererReady() {
  ...
  setTimeout(() => rendererVisible.value = !!renderer.value.contentWindow.document.getElementById('b24ui-devtools-renderer'), 500)
}
...
const previewUrl = computed(() => {
  if (!props.component) return
  const baseUrl = `/__bitrix24_ui__/components/${props.component.slug}`
...
<template>
  <div class="flex flex-col bg-grid">
    ...
    <div v-if="!rendererVisible" class="grow w-full flex justify-center items-center px-8">
      <B24Alert color="error" variant="subtle" title="Component preview not found" icon="i-lucide-circle-alert">
