...
const component = useState<Component | undefined>('__b24ui-devtools-component')
const state = useState<Record<string, any>>('__b24ui-devtools-state', () => ({}))

const { data: components, status, error } = useAsyncData<Array<Component>>('__b24ui-devtools-components', async () => {
...
function updateRenderer() {
  if (!component.value) return
  const event: Event & { data?: any } = new Event('bitrix24-ui-devtools:update-renderer')
  event.data = {
...
onMounted(() => window.addEventListener('bitrix24-ui-devtools:component-loaded', onComponentLoaded))
onUnmounted(() => window.removeEventListener('bitrix24-ui-devtools:component-loaded', onComponentLoaded))
...
const isDark = computed({
  ...
  set(value) {
    ...
    const event: Event & { isDark?: boolean } = new Event('bitrix24-ui-devtools:set-color-mode')
    event.isDark = value
    window.dispatchEvent(event)
  }
})
...
<template>
  <B24App class="flex justify-center items-center h-screen w-full relative font-sans">
    <div v-if="status === 'pending' || error || !component || !components?.length">
      <div v-if="error" class="flex flex-col justify-center items-center h-screen w-screen text-center text-[var(--ui-color-error-500)]">
        <B24UILogo class="h-8" />
        <B24Icon name="i-lucide-circle-alert" size="20" class="mt-2" />
        <p>
      ...
      >
        ...
        <div class="absolute top-[49px] bottom-0 inset-x-0 grid xl:grid-cols-8 grid-cols-4 bg-[var(--ui-bg)]">
          ...
          <div class="border-l border-[var(--ui-border)] flex flex-col col-span-2 overflow-y-auto">
            <B24Tabs color="neutral" variant="link" :items="tabs" class="relative" :ui="{ list: 'sticky top-0 bg-[var(--ui-bg)] z-50' }">
              <template #props>
