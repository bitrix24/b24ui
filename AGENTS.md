...
- **Releases are automated**: merging work into `main` publishes nothing; release-please keeps one release PR open and merging *that* tags, releases and publishes to npm. Version arithmetic (`feat` -> minor, `fix` -> patch), the release cadence commitment, the `severity:crash` hotfix policy and the release App the workflow needs to open a *mergeable* PR live in [releasing.md](.github/contributing/releasing.md). Do not "simplify" the token wiring in `.github/workflows/release-please.yml` — it looks redundant and is not; `test/workflows/release-wiring.test.sh` explains why by failing. Filed here rather than in the References table below, which is scoped to `src/` and `test/` work.
