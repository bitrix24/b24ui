...
- **Upstream names and patterns that a port would silently restore.** Two fixes
  in PR #335 are one faithful port away from being undone, and neither would go
  red on its own:
  - `src/templates.ts` generates **`b24ui.css`**, not upstream's `ui.css`. The
    `componentDetection` dev watcher filters `updateTemplates` on that name, and
    a filter matching nothing is a successful call — the feature just quietly
    stops working. Both halves now read `CSS_TEMPLATE_FILENAME` / `isCssTemplate`
    from one place; port the intent, never the literal.
  - `ChatMessages.updateLastMessageHeight()` reads
    `getBoundingClientRect().height` directly. It must **not** go back to
    `useElementBounding()`: the function runs from a watcher and a `resize`
    handler, outside any effect scope, so the composable leaks a ResizeObserver
    per call.
  Both are guarded — `test/utils/templates.spec.ts` and the ResizeObserver-count
  case in `test/components/ChatMessages.spec.ts`.
...
- 2026-08-08 — fixes of #79/#80/#81/#83 (PR #335): added the §2 **upstream names and patterns that a port would silently restore** invariant, covering the generated CSS template name (`b24ui.css`, not upstream's `ui.css`) and `ChatMessages`' direct `getBoundingClientRect()` read. Both were found by the June 2026 audit rather than by a failing test, because both fail silently: a template filter that matches nothing still returns, and a leaked ResizeObserver costs memory rather than correctness. Now guarded by `test/utils/templates.spec.ts` and a ResizeObserver-count case. Last reviewed: 2026-08-08.
