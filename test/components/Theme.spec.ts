...
describe('Theme', () => {
  ...
  // Without a `<B24Theme :props>` ancestor the component falls back to its own
  // `withDefaults` value. Tooltip declares `arrow: true`, so the arrow renders
  // even without a wrapping `<B24Theme>`. The proxy must not swallow that default
  // — Vue's auto-cast `false` for unset Boolean props is gated by the
  // `withDefaults` presence check inside the proxy.
  test('bare component uses withDefaults value for boolean prop', async () => {
    const wrapper = await mountSuspended({
  ...
  // `useFormField` reads raw `_props` and short-circuits to `'air-primary-alert'`
  // when a validation message is present, so the proxy fallback in
  // `color: color.value ?? props.color` never runs.
  test('FormField validation error overrides :props color', async () => {
    const wrapper = await mountSuspended({
      components: { Theme, FormField, Checkbox },
      template: `
        <Theme :props="{ checkbox: { color: 'air-primary-success' } }">
    ...
    expect(wrapper.html()).not.toContain('style-filled-success')
  })
  ...
  // `false` — otherwise `<B24Theme :props>` never reaches any form control.
  test(':props highlight reaches a form control', async () => {
    ...
    // `highlight` swaps the ring colour from the resting base to the variant's.
    expect(wrapper.find('button[role="checkbox"]').classes()).toContain('ring-(--b24ui-border-color)')
    expect(wrapper.find('button[role="checkbox"]').classes()).not.toContain('ring-(--ui-color-base-5)')
  })
  ...
  test(':props disabled disables a control instead of only styling it', async () => {
    ...
    expect(wrapper.find('[data-slot="base"]').attributes('aria-disabled')).toBe('true')
    expect(wrapper.find('[data-slot="base"]').attributes('tabindex')).toBe('-1')
  })

  // `<B24Form disabled>` is the closer context and must keep winning over an
  ...
  //
  // This is also why the group no longer carries a `withDefaults` colour: a
  // non-`undefined` default on the raw props short-circuits the
  // `formFieldColor.value ?? props.color` chain before the proxy is ever read,
  // so `<B24Theme :props>` could not win. `src/theme/checkbox.ts` supplies the
  // same `air-primary` through `defaultVariants`, which is the layer the
  // documented precedence puts it in.
  test(':props color on a checkbox group reaches its items', async () => {
    const wrapper = await mountSuspended({
      components: { Theme, CheckboxGroup },
      template: `
        <Theme :props="{ checkboxGroup: { color: 'air-primary-success' } }">
  ...
  // validation error has to beat `<B24Theme :props>` on the items too, not just
  // on the group root.
