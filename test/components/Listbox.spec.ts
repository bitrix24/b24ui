...
describe('Listbox', () => {
  ...
  renderEach(Listbox, [
    ...
    ['with selectedIcon', { props: { ...props, selectedIcon: Cross30Icon } }],
    ...
    ['with b24ui', { props: { ...props, b24ui: { content: 'p-2' } } }],
    // Slots
  ...
  describe('form integration', async () => {
    ...
    async function createForm(validateOn?: FormInputEvents[]) {
      const wrapper = await renderForm({
        ...
        slotTemplate: `
        <B24FormField name="value" label="Listbox">
          <B24Listbox id="input" v-model="state.value" :items="items" multiple />
        </B24FormField>
        `
      })
      const input = wrapper.findComponent({ name: 'ListboxRoot' })
