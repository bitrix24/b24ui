...
describe('SelectMenu', () => {
  ...
  // Reka's `ComboboxTrigger` hard-codes `aria-label="Show popup"`, which would win over the `B24FormField` label.
  describe('accessible name', () => {
    ...
    test('is named by the form field label', async () => {
      const wrapper = await renderForm({
        slotTemplate: `
        <B24FormField label="Sort order">
          <B24SelectMenu :items="['Newest', 'Oldest']" />
        </B24FormField>
        `
      })
