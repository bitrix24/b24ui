...
describe('InputMenu', () => {
  ...
  describe('clear', () => {
    ...
    it('does not render the clear button when disabled', async () => {
      const wrapper = await mountSuspended(InputMenu, { props: { items, modelValue: items[0], clear: true, disabled: true } as any })

      expect(clearButton(wrapper).exists()).toBe(false)
    })

    it('renders the clear button when not disabled', async () => {
      const wrapper = await mountSuspended(InputMenu, { props: { items, modelValue: items[0], clear: true } as any })
    ...
    it('does not render the clear button when the form is disabled', async () => {
      const wrapper = await renderForm({
        props: { disabled: true },
        slotVars: { items },
        slotTemplate: `<B24FormField name="value"><B24InputMenu :items="items" :model-value="items[0]" clear /></B24FormField>`
      })
  ...
  describe('multiple', () => {
    ...
    // The `!searchTerm` half of the guard: with nothing typed there is no tag to add, and
    // preventing the key would also swallow the implicit form submission an empty field allows.
    test('does not prevent enter when nothing has been typed', async () => {
      const wrapper = mount(InputMenu, {
        attachTo: document.body,
        props: {
          modelValue: ['Option 1'],
      ...
      input.element.addEventListener('keydown', event => (prevented = event.defaultPrevented))
