...
describe('Range', () => {
  ...
  describe('aria', () => {
    async function renderThumbs(options: { props?: any, attrs?: any } = {}) {
      const wrapper = await mountSuspended(Range, options)
    ...
    test('does not group an unlabelled range', async () => {
    ...
    test('keeps non-aria attributes on the root', async () => {
      const { wrapper, thumbs } = await renderThumbs({ props: { modelValue: 10 }, attrs: { 'data-testid': 'range' } })

      expect(wrapper.get('[data-slot="root"]').attributes('data-testid')).toBe('range')
    ...
    test('tracks aria attributes changed after mount', async () => {
      ...
      const Parent = defineComponent({
        setup: () => () => h(Range, { 'modelValue': 10, 'aria-label': label.value })
      })
    ...
    test('tracks aria attributes added after mounting without any', async () => {
      ...
      const Parent = defineComponent({
        setup: () => () => h(Range, { modelValue: 10, ...extra.value })
      })

      const wrapper = await mountSuspended(Parent)
      expect(wrapper.get('[role="slider"]').attributes('aria-label')).toBe('Thumb')

      extra.value = { 'aria-label': 'Volume', 'data-testid': 'range' }
      await nextTick()
      await nextTick()
    ...
    test('keeps a caller role on a grouped range', async () => {
    ...
    test('merges the form aria attributes with a caller label on the thumb', async () => {
      const wrapper = await mountSuspended(FormField, {
        props: { error: 'Error' },
        slots: { default: () => h(Range, { 'modelValue': 10, 'aria-label': 'Volume' }) }
      })
