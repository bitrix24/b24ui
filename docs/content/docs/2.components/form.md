...
| `getErrors(path?: keyof T \| RegExp)`{lang="ts-type"} | `FormError[]`{lang="ts-type"} <br> <div class="text-(--ui-color-design-tinted-na-content) mt-1"><p>Retrieves form errors associated with a specific path. If no path is provided, returns all form errors.</p></div> |
...
| `errors`{lang="ts-type"} | `Ref<FormError[]>`{lang="ts-type"} <br> <div class="text-(--ui-color-design-tinted-na-content) mt-1"><p>A reference to the array containing validation errors. Use this to access or manipulate the error information.</p></div> |
