...
::: info
When orientation is `vertical`, a [Accordion](/docs/components/accordion/) component is used to display each group. You can control the open state of each item using the `open` and `defaultOpen` properties and change the behavior using the [`collapsible`](/docs/components/accordion/#collapsible) and [`type`](/docs/components/accordion/#multiple) props.
:::
