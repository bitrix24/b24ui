...
During `nuxt dev` the scan re-runs whenever you save a `.vue`, `.ts`, `.js`, `.tsx` or `.jsx` file, so a component used for the first time picks up its styles without restarting the dev server.
