...
Use the `prose` option to force the import of Bitrix24 UI `<Prose>` components even if `@nuxtjs/mdc` or `@nuxt/content` is not installed.
...
Use the `content` option to force the import of Bitrix24 UI `<Prose>` and `<B24Content>` components even if `@nuxt/content` is not installed.
...
During `nuxt dev` the scan re-runs whenever you save a `.vue`, `.ts`, `.mts`, `.js`, `.mjs`, `.cjs`, `.tsx` or `.jsx` file, so a component used for the first time picks up its styles without restarting the dev server.
