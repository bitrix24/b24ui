...
You can configure the color-mode behavior through the `b24ui` module options in your `nuxt.config.ts`:
...
All these options are also available in your app’s `app.config.ts` (they are merged with the module options).
