...
#nuxt
:::warning
Set a slot to a function in one place only. Nuxt merges the `app.config.ts` of every [layer](https://nuxt.com/docs/getting-started/layers) with `defuFn`, which calls your function with whatever value the key already has lower down and keeps only the result. Bitrix24 UI then reads a plain string, so its classes are merged onto the defaults instead of replacing them.
:::
::
