...
Two theme variables cap how tall a popup may grow. Both are ceilings: the
component still takes the smaller of the ceiling and the space the popper
actually has, so a popup near the edge of the viewport shrinks rather than
overflowing, and its content scrolls.

| Variable | Default | Applies to |
|---|---|---|
| `--max-height-popup-list` | `40rem` | `Select`, `SelectMenu`, `InputMenu` — scrollable result lists |
| `--max-height-popup-menu` | `40vh` | `DropdownMenu`, `ContextMenu` — action menus |
