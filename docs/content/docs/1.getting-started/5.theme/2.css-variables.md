...
Bitrix24 UI provides Tailwind CSS utility classes for each [semantic color](/docs/getting-started/theme/design-system/#semantic-colors) you define, allowing you to use class names like `text-error` or `bg-success`:
...
Bitrix24 UI provides Tailwind CSS utility classes for text colors, allowing you to use class names like `text-dimmed` or `text-muted`:
...
Bitrix24 UI overrides Tailwind CSS's default `rounded-*` utilities with a unified border radius system, allowing you to use regular [border radius utilities](https://tailwindcss.com/docs/border-radius) like `rounded-xs` or `rounded-2xl`:
...
Bitrix24 UI provides a `--ui-container` CSS variable that controls the maximum width of the [Container](/docs/components/container/) component.
...
Bitrix24 UI provides a `--ui-header-height` CSS variable that controls the height of the Header component.
...
Bitrix24 UI applies default classes on the `<body>`{lang="html"} element of your app for consistent theming across light and dark modes:
