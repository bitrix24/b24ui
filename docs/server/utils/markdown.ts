 ...
 * through untouched.
 *
 * Links stay as written. Upstream can say that because `nuxt-agent-discovery`
 * absolutises the stringified document afterwards; here the same job is done
 * by `processLinks` in `transformMDC.ts`, which walks the tree and rewrites
 * `a.href` in place. That is why the table pass has to run *after* it — see
