 ...
 * `b24-ui-get-component-metadata` tool and the `b24-ui-implement-component-with-props`
 * prompt. Returns `null` when no meta exists for the component.
 */
export async function fetchComponentMetadata(normalizedName: string, { full = false }: { full?: boolean } = {}) {
  ...
  const componentMetaName: string = `B24${upperFirst(camelName)}`
 ...
 * `active` since Button documents `active` and `activeColor` as first-class props.
 */
